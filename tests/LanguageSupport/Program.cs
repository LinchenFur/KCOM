...
internal static partial class Program
{
    ...
    static void TestResourceInventory()
    {
        ...
        var clients = new List<IndexedClient> { source };
        Player? sourcePlayer = AlyxGlobalData.instance.AddPlayer(source);
        sourcePlayer!.InitializationStage = InitializationStage.Ready;
        void Feed(IWebSocketConnection socket, string line) => _ = new AlyxGamemode.AlyxGamemode(GamemodeHandleType.PreResponse,
            new Response("print", line), clients, socket, "mp_kiwitest");
        AlyxGamemode.AlyxGamemode.ResetResourceInventory();
        KiwisCoOpModCore.ResourceInventorySettings.SetShared(true);
        Feed(sourceSocket, "RESC 2 1 4 10 KCOM");
        clients.Add(peer);
        Player? peerPlayer = AlyxGlobalData.instance.AddPlayer(peer);
        peerPlayer!.InitializationStage = InitializationStage.Ready;
        Check(sourceProxy.Sent.Any(r => r.data == "kcom_setresources 2 1 4 10"), "shared resource snapshot broadcast");
        sourceProxy.Sent.Clear();
        peerProxy.Sent.Clear();
        Feed(peerSocket, "RESC 8 2 3 5 KCOM");
        Check(sourceProxy.Sent.Any(r => r.data == "kcom_setresources 2 1 4 10"), "new player does not replace shared baseline");
        sourceProxy.Sent.Clear();
        peerProxy.Sent.Clear();
        Feed(peerSocket, "RESC 3 1 4 10 KCOM");
        Check(sourceProxy.Sent.Any(r => r.data == "kcom_setresources 3 1 4 10"), "shared resource delta uses authoritative baseline");
        sourceProxy.Sent.Clear();
        peerProxy.Sent.Clear();
        Feed(sourceSocket, "RESC -1 1 4 8 KCOM");
        Check(sourceProxy.Sent.Count == 0 && peerProxy.Sent.Count == 0, "negative resource snapshot rejected");
        AlyxGamemode.AlyxGamemode.ResetResourceInventory();
        KiwisCoOpModCore.ResourceInventorySettings.Shared = false;
        sourceProxy.Sent.Clear();
        peerProxy.Sent.Clear();
        Feed(sourceSocket, "RESC 4 1 4 8 KCOM");
